...
This is a Request for Comments on a discovery protocol for the new ASCOM Alpaca platform that enables clients to find Alpaca servers within a Local Area Network (LAN). It is based on UDP and is designed to be as light weight and easy to implement as possible. It uses a known broadcast port and a known message / response. All information about the servers can then be retrieved via the management API.
